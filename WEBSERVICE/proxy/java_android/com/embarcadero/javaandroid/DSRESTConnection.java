...
public class DSRESTConnection {
	...
	/**
	 * Returns a new instance of HttpClient. If connectionTimeout or communicationTimeout have been set,
	 * then this client will be using those values.
	 * 
	...
	private HttpClient getHttpClient()
	{
		if (communicationTimeout > 0 || connectionTimeout > 0) {
			...
			if (communicationTimeout > 0)
			   HttpConnectionParams.setSoTimeout(httpParameters, communicationTimeout);
	...
	private void throwExceptionIfNeeded(HttpResponse response)
		...
		if (response.getStatusLine().getStatusCode() != 200) {
			...
			try {
				json = TJSONObject.Parse(EntityUtils
